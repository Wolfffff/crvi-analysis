...
setModelMatrices = function(current_tissue, y, covariates){
    ...
    mod1 <- model.matrix(~1+treatment, cov) 
    mod0 <- model.matrix(~1, cov) 
...
setSVA = function(data){
     ...
     data$sva <- future_map2(counts_list[data$n.sva!=0], data$n.sva[data$n.sva!=0],
                             \(x, y) sva(x, data$mod1, data$mod0, n.sv=y))
     data
}
...
rnaseq_data <- import("cache/rnaseq_all_2023-12-04.rds")
covariates  <- import("cache/covariates_2023-12-04.rds")
